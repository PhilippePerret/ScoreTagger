...
def assemble_systems
  ...
  code = CODE_APPEND_FIRST_SYSTEM % {img1_name: "score-000.jpg", gap: SYSTEMS_SEPARATOR}
  code = <<-BASH
cd "#{FACTORY_FOLDER}"
#{code}
...
BASH
  ...
  FileUtils.move(score_expanded_path, File.join(SCORE_FOLDER,'score_expanded.jpg'))

  # On enregistre les tops dans le fichier de données
  Score.set(pages: [TOPS])
