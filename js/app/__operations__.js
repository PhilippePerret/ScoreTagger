...
window.openAnalyse = async function openAnalyse(folder, options = {
  ...
  __end("Fin de l'ouverture de l'analyse","openAnalyse [segment]", {output:true})
  return score
}
...
***/
window.drawAnalyse = async function(score, options = {}){
  TableAnalyse.resetAll()
  score.drawFirstPage()
  const allsystems = await score.loadSystemsPrepared()
  await score.instanciateAndPoseAllSystems(allsystems)
